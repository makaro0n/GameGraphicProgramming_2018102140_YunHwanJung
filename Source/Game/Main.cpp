...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    std::shared_ptr<CustomCube> customCube = std::make_shared<CustomCube>(L"pepe.dds");
    if (FAILED(game->GetRenderer()->AddRenderable(L"CustomCube", customCube)))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetVertexShaderOfRenderable(L"CustomCube", L"PhongShader")))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetPixelShaderOfRenderable(L"CustomCube", L"PhongShader")))
    {
        return 0;
    }
